    ...
    <>
      ...
      <Panel>
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-3.5 border-b border-white/8">
          <h2 className="text-[13.5px] font-bold text-white">Daftar Artikel</h2>
              ...
              <Row key={n.id}>
                <Cell className="max-w-[340px]">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="min-w-0">
                      <p className="font-bold text-white text-[12.5px] leading-snug line-clamp-2">{n.title}</p>
                ...
                <Cell>
                  <div className="flex gap-1.5">
                    <a href={`/news/${n.slug}`} target="_blank" rel="noreferrer" className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 text-slate-300 flex items-center justify-center transition-colors" title="Lihat di portal">
                      <ExternalLink className="w-3.5 h-3.5" />
                    </a>
                    <button onClick={() => openEdit(n)} className="w-8 h-8 rounded-lg bg-white/5 hover:bg-cyan-500/20 text-slate-300 hover:text-cyan-300 flex items-center justify-center transition-colors cursor-pointer" title="Ubah">
                      <Pencil className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => setDelId(n.id)} className="w-8 h-8 rounded-lg bg-white/5 hover:bg-rose-500/20 text-slate-300 hover:text-rose-300 flex items-center justify-center transition-colors cursor-pointer" title="Hapus">
      ...
      >
        <div className="space-y-4">
          ...
          {form.thumbnail && (
            <img src={form.thumbnail} alt="Pratinjau" className="w-full h-36 object-cover rounded-xl border border-white/10" onError={(e) => ((e.target as HTMLImageElement).style.display = 'none')} />
          )}
          ...
          <InfoNote>
            Isi berita mendukung tag HTML seperti <code className="text-cyan-300">&lt;p&gt;</code>, <code className="text-cyan-300">&lt;h2&gt;</code>, <code className="text-cyan-300">&lt;ul&gt;&lt;li&gt;</code>, dan <code className="text-cyan-300">&lt;blockquote&gt;</code> — semuanya otomatis mendapat gaya bernuansa penerbangan di halaman publik.
          </InfoNote>
