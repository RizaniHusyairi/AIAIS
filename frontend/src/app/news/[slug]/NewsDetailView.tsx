 ...
 *
 * BENTUKNYA. Halaman ini disusun seperti satu penerbangan, bukan seperti kartu
...
import {
  ...
  terpopuler, tetangga,
  waktuBaca, type Bagian,
} from '@/lib/berita';
import {
  Eye, ArrowLeft, ArrowRight, Plane, Clock, Link2, Check, MessageCircle, Share2,
  ChevronRight, ChevronDown, Sparkles, Newspaper, ListTree, ArrowUp, Compass, PenLine,
} from 'lucide-react';
...
export default function NewsDetailView({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <Penerbangan artikel={artikel} daftar={daftar} />;
}

/* ================================================================== */

function Penerbangan({ artikel, daftar }: { artikel: NewsItem; daftar: NewsItem[] }) {
  ...
  return (
    ...
    <div className="relative bg-[#f6f8fc]">
      ...
      <BilahKemajuan bacaanRef={bacaanRef} />

      <Hero artikel={artikel} menitBaca={menitBaca} heroRef={heroRef} kurangiGerak={kurangiGerak} />
 ...
 */
function Hero({
  artikel, menitBaca, heroRef, kurangiGerak,
}: {
  artikel: NewsItem;
  menitBaca: number;
...
function SobekanTiket({
  artikel, menitBaca, taut, tersalin, salinTaut,
}: {
  artikel: NewsItem;
  menitBaca: number;
  taut: string;
...
function RuteBaca({
  ...
  const lompat = (indeks: number) => {
    const el = judulKe(badanRef.current, indeks);
    if (!el) return;

    // Navbar `sticky` menutupi puncak layar; beri jarak supaya judul bagiannya
    // tidak tersembunyi di baliknya begitu berhenti bergulir.
